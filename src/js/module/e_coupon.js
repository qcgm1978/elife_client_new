elife.controller('ECouponCtrl',['$scope','$rootScope', '$routeParams', '$http', '$cookieStore', 'API', 'SharedState', function ($scope,$rootScope, $routeParams, $http, $cookieStore, API, SharedState){
  ...
  if (!ICBCUtil.isElifeAndroid() || !ICBCUtil.isElifeIos()) {
    $rootScope.token = '10001';
  }

  if($scope.eCode === undefined){ 
  API.getAllECoupons({}).then(function(data){
